...
const columns = [
  {
    title: "LOCATIONS",
    items: [
      { label: "Spark Podil", href: "/locations" },
      { label: "Spark Pechersk", href: "/locations" },
      { label: "Spark Osokorky", href: "/locations" },
    ],
    renderItem: (item: { label: string; href: string }) => (
  ...
  },
  {
    ...
    renderItem: (item: { label: string }) => (
      <p key={item.label} className={styles.colText}>
        {item.label}
      </p>
    ),
  },
...
export default function Footer() {
  const [open, setOpen] = useState(columns.map(() => false));
        ...
        <div className={styles.grid}>
          {columns.map((col, i) => (
            <div key={col.title}>
              ...
              <div
                className={`${styles.colLinks} ${open[i] ? styles.colLinksOpen : ""}`}
              >
                {col.items.map((item) => col.renderItem(item as never))}
              </div>
            </div>
          ))}
        </div>
        <div className={styles.bottom}>
          <p className={styles.copyright}>© 2026 Spark. All rights reserved.</p>
          <div className={styles.social}>
            <button className={styles.socialBtn}>
              ...
              >
                <path
                  d="M12.0001 16.4446C13.1788 16.4446 14.3093 15.9763 15.1428 15.1428C15.9763 14.3093 16.4446 13.1788 16.4446 12.0001C16.4446 10.8214 15.9763 9.69091 15.1428 8.85741C14.3093 8.02392 13.1788 7.55566 12.0001 7.55566C10.8214 7.55566 9.69091 8.02392 8.85741 8.85741C8.02392 9.69091 7.55566 10.8214 7.55566 12.0001C7.55566 13.1788 8.02392 14.3093 8.85741 15.1428C9.69091 15.9763 10.8214 16.4446 12.0001 16.4446Z"
                  stroke="currentColor"
                  strokeWidth="1.66667"
                  strokeLinecap="round"
                  strokeLinejoin="round"
